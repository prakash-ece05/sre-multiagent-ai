...
def get_success_rate_for_service(service: str, time_range: str) -> Dict:
    ...
    try:
        ...
        if data.get("status") == "success":
            ...
            if results:
                success_rate = float(results[0].get("value", [0, 0])[1])
                return {
                    "status": "success",
                    "service": service,
...
def get_p95_latency_for_service(service: str, time_range: str) -> Dict:
    ...
    try:
        ...
        if data.get("status") == "success":
            results = data.get("data", {}).get("result", [])
            if results:
                latency = float(results[0].get("value", [0, 0])[1])
                return {
                    "status": "success",
                    "service": service,
...
def get_p90_latency_for_service(service: str, time_range: str) -> Dict:
    """
    Get P90 latency for a given service and time range.
        ...
        if data.get("status") == "success":
            ...
            if results:
                ...
                return {
                    "status": "success",
                    "service": service,
                    "p90_latency_ms": round(latency, 2),
                    "time_range": time_range
                }
            return {
                "status": "success",
