...
from config.applications import get_application_kpis
...
def get_volume(capability: str, duration: str) -> Tuple[int, Optional[float]]:
    ...
    try:
        ...
        if data.get("status") == "success":  # Fixed: colon instead of semicolon
            ...
            if results:
                volume = float(results[0].get("value", [0, 0])[1])  # Fixed: proper indexing
                return response.status_code, volume
            return response.status_code, 0.0
        else:
            return response.status_code, "Failed API Request"  # Fixed: status_code
...
def get_availability(capability: str, duration: str) -> Tuple[int, Optional[float]]:
    ...
    query = (
        ...
        f'status="success"}}[{duration}])) / '  # Fixed: proper string formatting
    ...
    try:
        ...
        if data.get("status") == "success":  # Fixed: colon instead of semicolon
            results = data.get("data", {}).get("result", [])
            if results:
                availability = float(results[0].get("value", [0, 0])[1])  # Fixed: proper indexing
